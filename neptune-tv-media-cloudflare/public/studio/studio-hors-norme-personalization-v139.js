const RELEASE='neptune-studio-hors-norme-personalization-20260824-v139';
let lastOrderId='',loadingFor='';
...
function sync(){
  ...
  if(!orderId||['contenus','calendrier','finances'].includes(orderId))return;
  if(!tabs.querySelector('[data-hn-personalization-tab]')){
...
async function openPersonalization(orderId,button){
  ...
  try{
    ...
    render(body,result.personalization,orderId);
...
function render(host,p,orderId){
