...
source=source.replace("const snapshot=await evaluate(cdp,`(()=>({", `const liveImages=await evaluate(cdp,\`(()=>Array.from(document.querySelectorAll('.v132-offer-visual img')).map(img=>({src:img.getAttribute('src'),complete:img.complete,naturalWidth:img.naturalWidth})))()\`);assert(liveImages.some(x=>x.src==='${hors}'&&x.complete&&x.naturalWidth>0),'Copie exacte production Hors Norme non chargée');assert(liveImages.some(x=>x.src==='${libre}'&&x.complete&&x.naturalWidth>0),'Copie exacte production Libre non chargée');\n    const snapshot=await evaluate(cdp,\`(()=>({`);
