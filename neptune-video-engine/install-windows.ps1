...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
  Write-Host "Docker Desktop n'est pas installé ou n'est pas démarré." -ForegroundColor Yellow
  Start-Process "https://www.docker.com/products/docker-desktop/"
  throw "Installez Docker Desktop, redémarrez-le, puis relancez cet installateur."
}
...
Write-Host "Téléchargement de la version Neptune..."
...
Push-Location $installRoot
try {
  Write-Host "Construction du moteur local. Le premier lancement peut prendre plusieurs minutes..."
...
for ($attempt = 1; $attempt -le 60; $attempt++) {
  try {
    $headers = @{ 'X-Neptune-Engine-Token' = $token }
    $health = Invoke-RestMethod -Uri 'http://127.0.0.1:4318/health' -Headers $headers -TimeoutSec 4
    if ($health.ok) { $healthy = $true; break }
  } catch {}
  Start-Sleep -Seconds 2
}
if (-not $healthy) {
  throw "Le service est installé mais n'a pas répondu. Consultez Docker Desktop > neptune-video-engine."
}

Set-Clipboard -Value $token
Write-Host "Installation terminée." -ForegroundColor Green
Write-Host "Le code de connexion a été copié dans le presse-papiers et enregistré ici :" -ForegroundColor Cyan
