...
const BROWSER_LOCAL_RELEASE = 'neptune-browser-local-video-20260914-v1';

function openAiSemanticAssistPlugin() {
  return {
    name: 'neptune-openai-semantic-assist-browser-local',
    enforce: 'pre',
    transform(code, id) {
      ...
      if (transformed === code
        ...
        || !transformed.includes(BROWSER_LOCAL_RELEASE)) {
        throw new Error('Unable to activate Neptune browser-local semantic assist.');
      }
      if (transformed.includes('NeptuneVideoEngineBridge')
        || transformed.includes('processFileWithPermanentEngine')
        || transformed.includes('resumePermanentEngineJobs')) {
        throw new Error('Obsolete permanent Neptune Video Engine bridge was reintroduced.');
      }
