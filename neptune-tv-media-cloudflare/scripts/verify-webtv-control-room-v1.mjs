...
const [activeEntry38,activeEntry37,activeEntry36,activeEntry,appEntry,webtvEntry,legacyControl,nativeControl,media,directR2,legacyEncoder,nativeEncoder,html,ui,nativeUi,uploadUi,ia,navCompat,security,corsRaw,corsWorkflow,rootRaw,localRaw,rootPackageRaw,localPackageRaw]=await Promise.all([
...
expect(activeEntry38.includes("from './webtv-control-v118.js'"),'entry-v38 ne branche plus le moteur WebTV natif v118');
expect(activeEntry38.includes("url.pathname==='/api/public/webtv/state'"),'état public WebTV v118 absent');
expect(activeEntry38.includes("url.pathname.startsWith('/direct/live/')"),'proxy HLS public v118 absent');
expect(activeEntry38.includes("url.pathname==='/direct/'"),'lecteur direct Neptune v118 absent');
expect(activeEntry38.includes("const WIZARD_JS='/studio/client-passage-wizard-v118.js?v=1'"),'assistant Nouveau passage v118 non injecté');
expect(activeEntry38.includes("const WEBTV_NATIVE_JS='/studio/webtv-native-v118.js?v=1'"),'surcouche Studio WebTV native v118 non injectée');
...
expect(activeEntry36.includes('WebTvEncoder'),'entry-v36 ne réexporte plus WebTvEncoder historique');
...
expect(webtvEntry.includes('WebTvEncoder')&&webtvEntry.includes('maintainWebTv'),'le moteur Web TV historique n’est plus préservé sous v38');
...
for(const marker of ["import { Container, getContainer } from '@cloudflare/containers'",'YOUTUBE_RTMPS_URL','YOUTUBE_STREAM_KEY','getContainer(env.WEBTV_ENCODER, ENCODER_INSTANCE_NAME)',"'/api/admin/webtv/state'","'/api/admin/webtv/encoder'","url.protocol === 'rtmps:'"])expect(legacyControl.includes(marker),`contrat Web TV historique absent : ${marker}`);
for(const marker of ['STATE_KEY=\'webtv/control/state-v118.json\'','LEGACY_STATE_KEY=\'webtv/control/state-v1.json\'','provider:\'neptune\'','protocol:\'hls\'','manifestUrl:\'/direct/live/index.m3u8\'','youtube_start','youtube_stop','publicWebTvStateV118','proxyLiveAssetV118'])expect(nativeControl.includes(marker),`contrat WebTV natif v118 absent : ${marker}`);
expect(nativeControl.includes('configured:true,watchUrl:\'/direct/\''),'la chaîne Neptune ne peut plus être configurée indépendamment de YouTube');
expect(!nativeControl.includes("if(state.enabled&&!state.output.youtube.configured)"),'YouTube redevient un prérequis pour activer Neptune');
...
for(const marker of ['ffmpeg','ffprobe',"'-f', 'flv'",'rtmps://[youtube]','streamTarget(cfg)'])expect(legacyEncoder.includes(marker),`moteur FFmpeg historique incomplet : ${marker}`);
for(const marker of ['HLS_DIR','NATIVE_PORT=23000','YOUTUBE_PORT=23001','buildNativeRelayArgs','buildYoutubeArgs','-hls_time','index.m3u8','youtubeStatus'])expect(nativeEncoder.includes(marker),`moteur FFmpeg natif v118 incomplet : ${marker}`);
expect(!nativeEncoder.includes('VOTRE_CLE_YOUTUBE'),'une clé YouTube factice ou dangereuse est présente');
...
for(const marker of ['neptuneNativePreview','/direct/?embed=1','Démarrer aussi sur YouTube','Arrêter le direct YouTube','youtube_start','youtube_stop'])expect(nativeUi.includes(marker),`interface native v118 incomplète : ${marker}`);
...
console.log('WebTV v118 validée : v115 reste préservée, Neptune HLS est la sortie principale, YouTube est secondaire, le programme continu et la médiathèque R2 restent connectés.');
