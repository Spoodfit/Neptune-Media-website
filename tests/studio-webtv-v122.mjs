...
const [overviewJs,overviewCss,catalogCss,catalogV129,webtvJs,webtvCss,analytics,entry40,entry41,advanced,webtv,wrangler]=await Promise.all([
  ...
  read('public/studio/studio-catalog-marketplace-v129.js'),
...
expect(catalogCss.includes('body.v128-studio-marketplace .content')&&catalogCss.includes('width:100%!important'),'Catalogue Média utilise toute la largeur disponible');
expect(catalogCss.includes('#content .c98-layout{display:none!important'),'gestion détaillée masquée par défaut derrière la marketplace');
expect(catalogCss.includes('v128-catalog-admin-open #content .c98-layout{display:block!important'),'gestion détaillée réactivable à la demande');
expect(catalogCss.includes('.v128-city-chooser')&&catalogCss.includes('.v128-offer-grid'),'marketplace structurée par ville et offres');
expect(catalogV129.includes("neptune-studio-catalog-marketplace-20260820-v129-worker-enforced"),'release marketplace v129 active');
expect(catalogV129.includes('Toutes les villes'),'ville comme porte d’entrée principale');
expect(catalogV129.includes('Gérer les données ▾'),'administration secondaire disponible à la demande');
expect(catalogV129.includes('Coût fournisseur')&&catalogV129.includes("price('Coûtant'")&&catalogV129.includes("price('Préférentiel'")&&catalogV129.includes("price('Normal'"),'carte offre rapproche coût fournisseur et trois tarifs client');
expect(catalogV129.includes('/api/admin/media-catalog-v98/context'),'marketplace utilise la source de vérité Studio');
expect(catalogV129.includes("hash==='#programs'||Boolean(tab)||title.includes('catalogue')"),'marketplace se monte depuis le DOM réel sans dépendre du pathname historique');
expect(entry41.includes("from './entry-v40.js'"),'entry-v41 préserve intégralement le runtime v40');
expect(entry41.includes('/studio/studio-catalog-marketplace-v129.js?v=1'),'Worker injecte la marketplace canonique v129');
expect(entry41.includes('LEGACY_CATALOG_JS')&&entry41.includes('studio-catalog-marketplace-v126.js'),'Worker retire les couches Catalogue concurrentes');
expect(entry41.includes('X-Neptune-Catalog-Marketplace'),'Worker expose un marqueur de release Catalogue vérifiable');
expect(advanced.includes('/studio/media-catalog-loader-v104.js?v=3'),'advanced conserve le bootstrap Catalogue historique comme couche de données/admin');
...
for(const asset of ['studio-overview-v122.js','studio-overview-v122.css','webtv-control-room-v122.js','webtv-control-room-v122.css','webtv-analytics-v122.js'])expect(entry40.includes(asset),`v40 injecte ${asset}`);
...
expect(wrangler.includes('"main": "src/entry-v41.js"'),'entry-v41 est le Worker actif');
