...
const entry=await fs.readFile(path.join(root,'src/entry-v40.js'),'utf8');
const extension=await fs.readFile(path.join(root,'src/studio-operating-v135.js'),'utf8');
...
assert.ok(entry.includes("from './studio-operating-v135.js'"),'active v40 does not load v135 extension');
assert.ok(entry.includes('handleStudioOperatingStoreV135')&&entry.includes('injectStudioOperatingDocumentV135'));
assert.ok(extension.includes("WEBTV_LEGACY_SCRIPTS=['/studio/webtv-workspace-v1.js','/studio/webtv-control-room-v122.js']"));
...
const catalogReplacement="if(event.target.matches('[data-v132-search]')){state.query=event.target.value;renderCatalogResultsV135();}";
...
async function moduleScript(page,source){
...
async function testCatalogTyping(){
  ...
  await moduleScript(page,hardenedCatalog);
  ...
  assert.equal(await page.evaluate(()=>document.activeElement?.matches?.('[data-v132-search]')),true,'catalog search lost focus after input refresh');
  assert.deepEqual(errors,[],'catalog runtime emitted browser errors');
  await page.close();
}
...
async function testWizardAndAgenda(){
  ...
  await page.route('**/studio/clients',route=>route.fulfill({status:200,contentType:'text/html',body:'<!doctype html><html><body><div class="clients-top-actions"><button id="newClient" type="button" onclick="document.getElementById(\'newDialog\').showModal()">Nouveau passage</button></div><dialog id="newDialog"><form id="newOrder"></form></dialog><button id="refresh">Actualiser</button></body></html>'}));
  ...
  await moduleScript(page,hardenedWizard);
  ...
  await moduleScript(page,operating);
  ...
  assert.deepEqual(errors,[],'wizard/agenda runtime emitted browser errors');
  await page.close();
}
