(() => {
  ...
  function apply(){scheduled=false;if(!order?.reservationSnapshot)return;const region=document.querySelector('#ccDetailRegion');if(!region||region.hidden)return;const stage=region.dataset.stage||'';if(stage==='format')renderFormat(region);if(stage==='payment')renderPayment(region);if(stage==='date')renderDate(region);}
  function renderFormat(region){
    ...
    const layout=region.querySelector('.cc-v118-stage-layout');if(layout){layout.innerHTML=html;return;}body.outerHTML=`<div class="cc-v118-stage-layout">${html}</div>`;
  }
  function renderPayment(region){
    ...
    target.outerHTML=facts([['Statut','Paiement validé'],['Montant payé',money(paid.paidAmountCents,paid.currency)],['Dossier',paid.reference||order.reference||order.id||'Neptune Media'],['Référence snapshot',order.reservationSnapshot?.capturedAt?dateTime(order.reservationSnapshot.capturedAt):'Enregistrée']]);
  }
  function renderDate(region){
    ...
    target.outerHTML=facts([['Date actuelle',r.currentDate?dateOnly(r.currentDate):'À confirmer'],['Créneau',daypart(r.currentDaypart)],['Date réservée initialement',initial.initialDate?dateOnly(initial.initialDate):'—'],['Statut',status]]);
  }
  function facts(items){return `<div class="cc-v118-facts">${items.map(([label,value])=>`<article><span>${esc(label)}</span><strong>${esc(value||'—')}</strong></article>`).join('')}</div>`;}
