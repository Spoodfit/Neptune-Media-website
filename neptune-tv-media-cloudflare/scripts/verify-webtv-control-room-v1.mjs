...
for(const marker of ['Diffusion','Web TV active','Ordre de passage','Programme de secours','Redémarrer l’encodeur','YouTube · RTMPS','webtv-upload-v4.js','webtv-upload-v1.css'])expect(html.includes(marker),`interface Diffusion incomplète : ${marker}`);
...
console.log('WebTV validée : import émission par PUT présigné unique navigateur→R2 jusqu’à 5 GiB, progression XHR, reprise réseau, vérification de taille, CSP/CORS R2, bibliothèque persistante, programme, Container FFmpeg et RTMPS préservés.');
