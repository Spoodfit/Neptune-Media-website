...
export function countPending(store,id=''){const row=id?store.sql.exec("SELECT COUNT(*) AS count FROM portal_email_outbox WHERE order_id=? AND status IN ('pending','failed')",id).one():store.sql.exec("SELECT COUNT(*) AS count FROM portal_email_outbox WHERE status IN ('pending','failed')").one();return Number(row?.count||0);}
