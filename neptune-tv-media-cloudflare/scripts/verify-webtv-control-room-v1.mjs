...
const [entry44,entry40,entry39,entry38,entry37,entry36,entry35,entry34,entry33,control,nativeControl,media,directR2,encoder,html,ui,nativeUi,uploadUi,ia,navCompat,security,corsRaw,corsWorkflow,rootRaw,localRaw,rootPackageRaw,localPackageRaw]=await Promise.all([
...
for(const marker of ['ffmpeg','ffprobe','HLS_DIR','buildNativeRelayArgs','buildYoutubeArgs','nativeHlsReady','youtubeStatus',"'-f','hls'","'-f','flv'",'NATIVE_OUTPUT','YOUTUBE_OUTPUT','youtubeTarget(cfg)'])expect(encoder.includes(marker),`moteur WebTV canonique incomplet : ${marker}`);
