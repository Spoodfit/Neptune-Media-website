...
async function measure(page, label, selectors) {
  ...
  for (const item of result.items) {
    ...
    else if (item.display === 'none' || item.visibility === 'hidden') fail(`${label}: élément essentiel masqué ${item.selector}.`);
  }
  return result;
}
...
async function auditClient(viewport) {
  ...
  const auth = await measure(page, `auth-${viewport.width}x${viewport.height}`, ['.auth-copy', '.access-card']);
  ...
  const dashboard = await measure(page, `dashboard-${viewport.width}x${viewport.height}`, ['.dashboard-heading', '.overview-grid', '.production-card', '.show-card', '.metric-grid', '.dashboard-content-grid']);
...
async function auditStudioClients(viewport) {
  ...
  if (hero && !hero.missing && (hero.height < 108 || hero.height > 195)) fail(`studio-clients-${viewport.width}x${viewport.height}: synthèse principale mal dimensionnée (${hero.height}px).`);
...
async function auditStudioAdvanced(viewport) {
  ...
  await page.waitForTimeout(100);
  const app = await measure(page, `studio-advanced-${viewport.width}x${viewport.height}`, ['.sidebar', '.topbar', '.page-intro', '.cards', '.task-grid']);
...
console.log('Adaptive interfaces v58 validated: client auth/dashboard/calendar and Studio clients/advanced.');
