...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && response.ok && isStudioClientsPath(url.pathname)
      ...
      response = await injectStudioOperations(response);
    }
...
async function injectStudioOperations(response) {
  ...
  body = body.replace('</head>', `<link rel="stylesheet" href="${GALLERY_CSS}"><link rel="stylesheet" href="${OPERATIONS_CSS}"></head>`);
  body = body.replace('</body>', `<script type="module" src="${GALLERY_JS}"></script><script type="module" src="${OPERATIONS_JS}"></script></body>`);
