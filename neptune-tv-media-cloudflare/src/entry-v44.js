...
const STUDIO_CSS='/studio/studio-catalog-commercial-cockpit-v144.css?v=1';
const STUDIO_JS='/studio/studio-catalog-commercial-cockpit-v144.js?v=1';
...
async function inject(response,css,js,kind){
  ...
  const headers=new Headers(response.headers);for(const name of ['Content-Length','Content-Encoding','ETag','Last-Modified'])headers.delete(name);headers.set('Cache-Control','private, no-store, max-age=0');headers.set('X-Neptune-Catalog-Commerce',CATALOG_COMMERCE_V143_RELEASE);headers.set('X-Neptune-City-Drawer','v143.4');headers.set('X-Neptune-Catalog-Cockpit','v144');return new Response(body,{status:response.status,statusText:response.statusText,headers});
}
function mark(response){const headers=new Headers(response.headers);headers.set('X-Neptune-Catalog-Commerce',CATALOG_COMMERCE_V143_RELEASE);headers.set('X-Neptune-City-Drawer','v143.4');headers.set('X-Neptune-Catalog-Cockpit','v144');return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
