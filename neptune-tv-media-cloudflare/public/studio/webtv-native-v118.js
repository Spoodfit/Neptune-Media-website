...
function installUi(){
  ...
  const settings=[...document.querySelectorAll('.settings-card')].find(x=>x.querySelector('#enabled'));if(settings){
    ...
    const destination=settings.querySelector('.readonly-row');if(destination)destination.innerHTML='<span><b>Destination principale</b><small>Flux HLS Neptune · indépendant de YouTube</small></span><strong>media.neptunebusiness.com/direct/</strong>';
