...
async function injectClientExperience(response){
  ...
  body=body.replace(/<link\b[^>]*href=["'][^"']*\/espace-client\/(?:client-experience-v117|client-command-center-v118|client-catalog-rail-v118|client-visual-coherence-v118-2)\.css[^"']*["'][^>]*>\s*/giu,'');
  ...
  body=body.replace('</head>',`<link rel="stylesheet" href="${CLIENT_CSS}"><link rel="stylesheet" href="${COMMAND_CSS}"><link rel="stylesheet" href="${CATALOG_RAIL_CSS}"><link rel="stylesheet" href="${VISUAL_CSS}"></head>`);
...
async function augmentRelease(response){
  ...
  return new Response(JSON.stringify({...current,clientExperience:RELEASE,clientCommandCenter:'contextual-stage-details-content-folders-v118.2',clientPreparation:'step-local-reading-ack-v118',clientPreparationBridge:'v77-state-context-bridge-v118',clientCatalogVisuals:'studio-synced-v118',clientCatalogLayout:'city-first-horizontal-rail-v118.2',clientVisualCoherence:'selected-stage-support-spacing-snapshot-retirement-v118.2',clientLoadingStates:'skeleton-error-retry-reduced-motion-v117'}),{
