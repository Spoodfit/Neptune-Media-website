...
export const SALES_PROSPECT_V121_RELEASE='neptune-sales-prospect-20260903-email-company-v174';
...
export async function startTunnelProspectV121(store,raw={}){
  ...
  const reservationToken=String(raw.reservationToken||raw.reservation_token||raw.token||'').trim();

  const reused=await reuseEmailGateProspect(store,{reservationToken,providedEmail,firstName,lastName,phone,company,companyIdentity,websiteHint,at});
  ...
  upsertCompanyContext(store,prospectId,companyIdentity,websiteHint,at);

  return successResponse({token,prospectId,firstName,lastName,fullName,company,companyIdentity,websiteHint,email:providedEmail||'',phone});
}
...
async function reuseEmailGateProspect(store,{reservationToken,providedEmail,firstName,lastName,phone,company,companyIdentity,websiteHint,at}){
  if(reservationToken.length<32)return null;
  let hash='';
  try{hash=await sha256(reservationToken);}catch{return null;}
  const row=store.sql.exec(`SELECT p.id,p.client_id AS clientId,p.email,p.status,p.expires_at AS expiresAt,c.email AS clientEmail,c.full_name AS fullName
    FROM portal_prospects p JOIN portal_clients c ON c.id=p.client_id
    WHERE p.token_hash=? LIMIT 1`,hash).toArray()[0]||null;
  if(!row||!['captured','tunnel_started'].includes(String(row.status||''))||String(row.expiresAt||'')<=at)return null;

  const clientEmail=normalizeEmail(row.clientEmail||row.email);
  if(providedEmail&&clientEmail&&providedEmail!==clientEmail&&!isPendingEmail(clientEmail))return null;

  const suppliedFullName=`${firstName} ${lastName}`.trim();
  store.sql.exec(`UPDATE portal_clients SET
      full_name=CASE WHEN ?<>'' THEN ? ELSE full_name END,
      company=?,active=1,updated_at=? WHERE id=?`,
    suppliedFullName,suppliedFullName,company,at,row.clientId);
  store.sql.exec(`UPDATE portal_prospects SET
      first_name=CASE WHEN ?<>'' THEN ? ELSE first_name END,
      last_name=CASE WHEN ?<>'' THEN ? ELSE last_name END,
      company=?,updated_at=? WHERE id=?`,
    firstName,firstName,lastName,lastName,company,at,row.id);
  store.sql.exec(`INSERT INTO portal_client_profiles_v96(client_id,phone,updated_at) VALUES(?,?,?)
    ON CONFLICT(client_id) DO UPDATE SET phone=CASE WHEN excluded.phone<>'' THEN excluded.phone ELSE portal_client_profiles_v96.phone END,updated_at=excluded.updated_at`,row.clientId,phone,at);
  upsertCompanyContext(store,row.id,companyIdentity,websiteHint,at);

  const fullName=suppliedFullName||String(row.fullName||'').trim()||company;
  return successResponse({
    ...
    companyIdentity,
    websiteHint,
    email:providedEmail||clientEmail||'',
    phone,
  });
}

function successResponse({token,prospectId,firstName,lastName,fullName,company,companyIdentity,websiteHint,email,phone}){
  return json({
    ok:true,
    ...
    contact:{firstName,lastName,fullName,company,companyIdentity,websiteHint,email,phone,enrichmentStatus:'pending'},
  });
}

function upsertCompanyContext(store,prospectId,companyIdentity,websiteHint,at){
...
function isPendingEmail(value){return String(value||'').toLowerCase().endsWith(`@${PENDING_EMAIL_DOMAIN}`);}
