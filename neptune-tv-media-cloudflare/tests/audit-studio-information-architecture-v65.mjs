...
try {
  for (const viewport of viewports) {
    for (const screen of screens) {
      ...
      await page.waitForSelector('.neptune-studio-nav-link');
      ...
      const metrics = await page.evaluate(() => {
        ...
        const links = [...document.querySelectorAll('.neptune-studio-nav-link')].filter(visible);
        return {
      ...
      assert(JSON.stringify(metrics.primaryTexts) === JSON.stringify(expectedPrimary), `${screen.id}/${viewport.id}: navigation principale incorrecte ${JSON.stringify(metrics.primaryTexts)}`);
      ...
      if (screen.id === 'diffusion') {
        assert(JSON.stringify(metrics.contextTexts) === JSON.stringify(['Programme', 'Formats', 'Publicités', 'Audience']), `diffusion/${viewport.id}: sous-navigation Diffusion incorrecte`);
      ...
      reports.push({ screen, viewport, metrics });
      await context.close();
    }
  }
} finally {
