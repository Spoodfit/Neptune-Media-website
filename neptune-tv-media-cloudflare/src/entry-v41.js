...
const CATALOG_RELEASE='neptune-studio-catalog-visual-20260820-v132';
...
async function injectVisualCatalog(response){
  ...
  body=body.replace('</head>',`<link rel="stylesheet" href="${V132_CSS}"></head>`);
  body=body.replace('</body>',`<script type="module" src="${V132_JS}"></script></body>`);
  const headers=new Headers(response.headers);
  for(const name of ['Content-Length','Content-Encoding','ETag','Last-Modified'])headers.delete(name);
  headers.set('Cache-Control','private, no-store, max-age=0');
  headers.set('X-Neptune-Catalog-Runtime',CATALOG_RELEASE);
...
async function augmentRelease(response){
  ...
  headers.set('X-Neptune-Catalog-Runtime',CATALOG_RELEASE);
  return new Response(JSON.stringify({...current,catalogRuntime:CATALOG_RELEASE}),{status:response.status,statusText:response.statusText,headers});
}
