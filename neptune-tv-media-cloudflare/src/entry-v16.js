...
const RELEASE = 'neptune-video-browser-local-20260914-v1';
...
async function augmentRelease(response, env) {
  ...
  return new Response(JSON.stringify({
    ...
    videoAiPipeline: 'browser-local-whisper-openai-workers-ai-local-render-studio-sync',
    videoAiEngineMode: 'browser-local',
    videoAiDispatchMode: 'browser-session-indexeddb',
    ...
    videoAiRendering: 'browser-local-vertical-rendering',
    videoAiTranscription: 'browser-whisper-local-primary',
    videoAiStorage: 'indexeddb-browser-local-review-sync',
    videoAiSemanticProvider: 'openai-then-workers-ai-then-neptune-rules',
    videoAiSemanticPriority: 'openai-then-workers-ai-then-neptune-rules',
    videoAiOpenAiConfigured: openAiConfigured,
    videoAiOpenAiModel: openAiModel(env),
    videoAiOpenAiMode: 'server-side-browser-assist',
    ...
    videoAiWorkersAiRole: 'fallback-when-openai-is-unavailable',
    ...
    videoAiBrowserLocalPrimary: true,
    videoAiReady: true,
    videoAiLegacyCloudRecovery: 'retired-reselect-source-into-browser-local-engine',
    videoAiCrossOriginIsolation: 'scoped-to-studio-video-ai-and-browser-local-assets',
    videoAiDriveTransport: 'approved-local-blob-streamed-directly-through-worker-to-drive',
    videoAiStudioExperience: 'browser-local-production-and-review',
...
async function injectStudioInformationArchitecture(response, pathname) {
  ...
  const retiredPermanentEngineJsPattern = /<script\b[^>]*src=["'][^"']*\/studio\/video-ai-engine-v73\.js[^"']*["'][^>]*>\s*<\/script>\s*/giu;
  const retiredGenerationPatchPattern = /<script\b[^>]*src=["'][^"']*\/studio\/video-ai-generation-v75\.js[^"']*["'][^>]*>\s*<\/script>\s*/giu;
  ...
  body = body.replace(retiredPermanentEngineJsPattern, '');
  body = body.replace(retiredGenerationPatchPattern, '');
