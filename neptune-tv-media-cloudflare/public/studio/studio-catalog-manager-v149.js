const RELEASE='neptune-studio-catalog-presentation-20260829-v149';
...
function boot(){
  const run=()=>{
    scheduleEnhance(0);
    new MutationObserver(()=>scheduleEnhance(20)).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden','class','open']});
    document.addEventListener('click',event=>{
      if(event.target?.closest?.('[data-v145-configure],[data-v145-add],[data-v147-manage],[data-v147-edit],[data-v147-list],[data-v147-new],[data-v147-back-list]'))scheduleEnhance(35);
    },true);
    document.addEventListener('submit',event=>{
      if(event.target?.closest?.(`#${DIALOG_ID}`)){
        contextCache=null;
        setTimeout(()=>{contextCache=null;scheduleEnhance(0)},650);
      }
    },true);
  };
...
function scheduleEnhance(delay=20){
  ...
  enhanceTimer=setTimeout(()=>enhance().catch(error=>console.warn('[catalog-v149]',error)),delay);
}
...
async function enhance(){
  ...
  const hasVisualList=Boolean(dialog.querySelector('[data-v147-edit="concept"],[data-v147-edit="physical"]'));
  if(concept||physical||hasVisualList){
    const ctx=await getContext();
    if(concept)enhanceConceptVisual(concept,ctx);
    if(physical)enhancePhysicalVisual(physical,ctx);
    if(hasVisualList)enhanceListVisuals(dialog,ctx);
  }
}

function enhanceOfferTabs(form){
  if(form.dataset.v149Tabs==='1')return;
  const configuration=form.querySelector(':scope > .v147-form');
  const pricing=form.querySelector(':scope > .v147-tiers');
  const header=form.querySelector(':scope > header');
  if(!configuration||!pricing||!header)return;
  form.dataset.v149Tabs='1';
  form.dataset.v149ActiveTab='configuration';
  configuration.dataset.v149Panel='configuration';
  pricing.dataset.v149Panel='pricing';
  pricing.hidden=true;

  const tabs=document.createElement('nav');
  tabs.className='v149-tabs';
  tabs.setAttribute('role','tablist');
  tabs.setAttribute('aria-label','Configuration de l’offre');
  tabs.innerHTML='<button type="button" role="tab" aria-selected="true" class="is-active" data-v149-tab="configuration"><b>1</b><span>Configuration</span><small>Ville, concept, formats et fournisseur</small></button><button type="button" role="tab" aria-selected="false" data-v149-tab="pricing"><b>2</b><span>Prix & Stripe</span><small>Tarifs, liens de paiement et quotas</small></button>';
  header.after(tabs);
  tabs.addEventListener('click',event=>{
    const button=event.target.closest('[data-v149-tab]');
    if(button)activateOfferTab(form,button.dataset.v149Tab);
  });
  ...
  if(feedback){
    new MutationObserver(()=>{
      ...
      if(/stripe|tarif|prix|paiement|lien|place|quota/.test(text))activateOfferTab(form,'pricing');
      else activateOfferTab(form,'configuration');
...
function activateOfferTab(form,name){
  for(const panel of form.querySelectorAll('[data-v149-panel]'))panel.hidden=panel.dataset.v149Panel!==name;
  for(const button of form.querySelectorAll('[data-v149-tab]')){
    const active=button.dataset.v149Tab===name;
    button.setAttribute('aria-selected',active?'true':'false');
    button.classList.toggle('is-active',active);
  }
  form.dataset.v149ActiveTab=name;
}
...
function createVisualPreview(url,title){
  ...
  root.className='v149-visual-preview wide';
...
function setPreview(root,image,copy,url,label){
  ...
  if(has){
    ...
    copy.innerHTML=`<strong>${escapeHtml(label)}</strong><span>L’image enregistrée est affichée ici. Sélectionnez un fichier ci-dessous uniquement pour la remplacer.</span>`;
    image.onerror=()=>{
      root.classList.add('is-empty');
      image.hidden=true;
      copy.innerHTML='<strong>Visuel enregistré mais indisponible</strong><span>Le fichier référencé ne peut pas être chargé. Remplacez-le pour corriger le catalogue.</span>';
    };
  }else{
    image.hidden=true;
...
function bindLiveFilePreview(file,preview,originalUrl){
  ...
  file.addEventListener('change',()=>{
    ...
    if(selected){
      ...
      setPreview(preview.root,preview.image,preview.copy,objectUrl,'Nouveau visuel sélectionné');
...
function enhanceListVisuals(dialog,ctx){
  for(const row of dialog.querySelectorAll('[data-v147-edit="concept"],[data-v147-edit="physical"]')){
    if(row.dataset.v149Thumb==='1')continue;
    ...
    if(!url)continue;
    row.dataset.v149Thumb='1';
    const thumb=document.createElement('span');
    thumb.className='v149-row-thumb';
