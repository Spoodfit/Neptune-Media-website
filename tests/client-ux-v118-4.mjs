...
tomorrow.setDate(tomorrow.getDate()+1);tomorrow.setHours(10,30,0,0);
const later=new Date(tomorrow);later.setDate(later.getDate()+2);later.setHours(15,0,0,0);
...
const files=[
  ...
  ...Array.from({length:8},(_,i)=>({id:`short-${i+1}`,driveFileId:`drive-short-${i+1}`,fileType:i?'short':'reel',name:`Sujet_Reel_${String(i+1).padStart(2,'0')}.mp4`})),
];
...
async function homeAudit(){
  ...
  const page=await context.newPage();const browserErrors=[];watch(page,browserErrors);await routes(page);
  try{
    await okGoto(page,`${baseUrl}/espace-client/?v1184=${Date.now()}`);
    await page.waitForSelector('#dashboard:not([hidden])',{timeout:20000});
    await page.evaluate(async()=>{
      for(const href of ['/espace-client/client-command-center-v118.css?v=1','/espace-client/client-catalog-rail-v118.css?v=1','/espace-client/client-visual-coherence-v118-2.css?v=1','/espace-client/client-ux-polish-v118-3.css?v=1','/espace-client/client-ux-v118-4.css?v=1']){const link=document.createElement('link');link.rel='stylesheet';link.href=href;document.head.append(link);}
    ...
    await page.waitForSelector('.cc-stage.is-selected-v118',{timeout:12000});await page.waitForTimeout(160);
    const selected=await page.evaluate(()=>{
      ...
      return {after:button?getComputedStyle(button,'::after').content:'',markerHeight:button?getComputedStyle(button,'::before').height:'',buttonShadow:button?getComputedStyle(button).boxShadow:'',iconShadow:icon?getComputedStyle(icon).boxShadow:''};
    });
    expect(['none','normal','""'].includes(selected.after),`ancien grand contour encore actif (${selected.after})`);
    ...
    await page.evaluate(()=>document.querySelector('.client-command-center')?.append(document.createComment('legacy-observer-check')));
    ...
    await page.locator('[data-v1184-detail-toggle]').click();await page.waitForTimeout(60);
    expect(await page.locator('#ccDetailRegion').evaluate(node=>getComputedStyle(node).display!=='none'),'détail impossible à rouvrir');
    if(browserErrors.length)throw new Error(browserErrors.join(' | '));
    await page.screenshot({path:path.join(out,'home-selection-detail.png'),fullPage:true});
  }catch(error){errors.push(`home: ${error.message}`);}finally{await context.close();}
}
...
async function libraryAudit(){
  ...
  try{
    ...
    await page.locator('[data-media-folder="short"]').click();await page.waitForTimeout(100);
    const d=await page.evaluate(()=>{
      ...
      return {shellW:Math.round(shell?.width||0),sectionW:Math.round(section?.width||0),gridDisplay:grid?getComputedStyle(grid).display:'',cards:cards.length,firstW:Math.round(first?.width||0),sameRow:Boolean(first&&second&&Math.abs(first.top-second.top)<4),overflow:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth)-innerWidth};
    });
    ...
    expect(d.cards===8,`${d.cards} shorts au lieu de 8`);expect(d.sameRow,'grille shorts non répartie sur la largeur');
    expect(d.firstW>150,`cartes shorts trop écrasées (${d.firstW}px)`);expect(d.overflow<=3,`overflow bibliothèque ${d.overflow}px`);
    if(browserErrors.length)throw new Error(browserErrors.join(' | '));
    await page.screenshot({path:path.join(out,'library-full-width.png'),fullPage:true});
  }catch(error){errors.push(`library: ${error.message}`);}finally{await context.close();}
}
...
async function calendarAudit(viewport,name){
  ...
  try{
    ...
    expect(week.title==='Que dois-je publier ?',`${name}: promesse calendrier incorrecte`);expect(week.weekActive,`${name}: semaine non active par défaut`);
    expect(!week.wrongText,`${name}: texte IA hors contexte encore visible`);expect(week.realText,`${name}: identité réelle vidéo absente`);
    expect(week.libraryHidden&&week.reuseHidden,`${name}: doublons historiques encore visibles`);expect(week.overflow<=3,`${name}: overflow semaine ${week.overflow}px`);
  ...
  }catch(error){errors.push(`${name}: ${error.message}`);}finally{await context.close();}
}

async function routes(page){
  // Register the generic client fallback first. Playwright resolves matching
  // routes in reverse registration order, so the precise mocks below win.
  ...
  await page.route('**/api/client/content-calendar/update',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,publishAt:tomorrow.toISOString(),title:'TITRE IA HORS CONTEXTE',description:'DESCRIPTION IA HORS CONTEXTE',hashtags:['business'],networks:['youtube','instagram'],caption:''})}));
