...
const STUDIO_RELIABILITY_JS='/studio/studio-catalog-user-reliability-v146.js?v=1';
...
async function maybeEnhance(request,response){
  ...
  if(request.method==='GET'&&type.includes('text/html')&&isStudio(url.pathname))return inject(response,STUDIO_CSS,[STUDIO_RELIABILITY_JS,STUDIO_JS],'studio');
...
async function inject(response,css,scripts,kind){
  ...
  const headers=new Headers(response.headers);for(const name of ['Content-Length','Content-Encoding','ETag','Last-Modified'])headers.delete(name);headers.set('Cache-Control','private, no-store, max-age=0');headers.set('X-Neptune-Catalog-Commerce',CATALOG_COMMERCE_V143_RELEASE);headers.set('X-Neptune-City-Drawer','v143.4');headers.set('X-Neptune-Catalog-Cockpit','v145');headers.set('X-Neptune-Catalog-Reliability','v146');return new Response(body,{status:response.status,statusText:response.statusText,headers});
}
function mark(response){const headers=new Headers(response.headers);headers.set('X-Neptune-Catalog-Commerce',CATALOG_COMMERCE_V143_RELEASE);headers.set('X-Neptune-City-Drawer','v143.4');headers.set('X-Neptune-Catalog-Cockpit','v145');headers.set('X-Neptune-Catalog-Reliability','v146');return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
