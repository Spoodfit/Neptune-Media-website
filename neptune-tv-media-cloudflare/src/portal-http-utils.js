...
export function normalizeOrderPayload(raw = {}, env = {}) {
  ...
  return {
    ...
    formatId: String(source.formatId || source.format_id || metadata.formatId || metadata.format_id || '').trim(),
    serviceId: String(source.serviceId || source.service_id || metadata.serviceId || metadata.service_id || '').trim(),
    supplierRateId: String(source.supplierRateId || source.supplier_rate_id || metadata.supplierRateId || metadata.supplier_rate_id || '').trim(),
    cityId: String(source.cityId || source.city_id || metadata.cityId || metadata.city_id || '').trim(),
    supplierId: String(source.supplierId || source.supplier_id || metadata.supplierId || metadata.supplier_id || '').trim(),
    offerId: String(source.offerId || source.offer_id || metadata.offerId || metadata.offer_id || '').trim(),
