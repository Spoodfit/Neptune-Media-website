...
const DOSSIER_RELEASE = 'neptune-studio-client-dossier-20260810-v88';
const DOSSIER_CSS = '/studio/client-dossier-v88.css?v=1';
const DOSSIER_JS = '/studio/client-dossier-v88.js?v=1';
...
async function augmentRelease(response) {
  ...
  return new Response(JSON.stringify({
    ...
    studioClientDossierHierarchy: 'current-state-next-action-dates-deliverables-v88',
    studioClientDossierDeliveryGuard: 'frontend-inventory-mirrors-backend-final-plus-short-v88',
    studioClientDossierChronologyGuard: 'preparation-before-filming-visible-warning-v88',
  }), {
    status: response.status,
    headers: {
...
async function injectStudioClientEnhancements(response) {
  ...
  body = body.replace(/<link\b[^>]*href=["'][^"']*\/studio\/client-dossier-v88\.css[^"']*["'][^>]*>\s*/giu, '');
  body = body.replace(/<script\b[^>]*src=["'][^"']*\/studio\/client-dossier-v88\.js[^"']*["'][^>]*>\s*<\/script>\s*/giu, '');
