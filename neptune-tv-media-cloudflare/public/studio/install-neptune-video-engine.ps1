...
Write-Host "Neptune Video Engine - installation / mise à jour" -ForegroundColor Cyan
...
$pairingPath = Join-Path $installRoot 'pairing.txt'
$envPath = Join-Path $installRoot '.env'

$existingToken = ''
$existingOpenAiKey = ''
if (Test-Path $pairingPath) {
  $existingToken = (Get-Content $pairingPath -Raw).Trim()
}
if (Test-Path $envPath) {
  $existingOpenAiLine = Get-Content $envPath | Where-Object { $_ -like 'OPENAI_API_KEY=*' } | Select-Object -First 1
  if ($existingOpenAiLine) { $existingOpenAiKey = $existingOpenAiLine.Substring('OPENAI_API_KEY='.Length) }
}

New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null
Write-Host "Téléchargement de la dernière version Neptune..." -ForegroundColor Cyan
...
if (Test-Path $installRoot) {
  Write-Host "Mise à jour du moteur existant sans supprimer les productions..." -ForegroundColor Yellow
  Push-Location $installRoot
  try { docker compose down | Out-Null } catch {}
  Pop-Location
  Get-ChildItem $installRoot -Force | Where-Object { $_.Name -notin @('pairing.txt', '.env') } | Remove-Item -Recurse -Force
} else {
  New-Item -ItemType Directory -Path $installRoot -Force | Out-Null
}
Copy-Item (Join-Path $engineSource '*') $installRoot -Recurse -Force
...
Set-Content -Path $pairingPath -Value $token -Encoding UTF8
...
Write-Host ("Moteur opérationnel : " + $health.version) -ForegroundColor Green
Write-Host "Le code de connexion est conservé et copié dans le presse-papiers." -ForegroundColor Green
Write-Host $pairingPath
