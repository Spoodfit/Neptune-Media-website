const DRIVE_SYNC_VERSION = 'neptune-drive-sync-v1';
...
function collecterFichiers_(folder, category, clientEmail, output) {
  ...
  while (iterator.hasNext()) {
    ...
    if (clientEmail) partagerLectureSansNotification_(file.getId(), clientEmail);
    output.push({
...
function partagerLectureSansNotification_(fileId, email) {
  ...
  const response = UrlFetchApp.fetch(url, {
    ...
    payload: JSON.stringify({ type: 'user', role: 'reader', emailAddress: normalized }),
  ...
  console.warn('drive_permission_warning', fileId, normalized, status, body.slice(0, 300));
}
